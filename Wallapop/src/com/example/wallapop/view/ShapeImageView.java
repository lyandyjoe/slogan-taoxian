...
package com.example.wallapop.view;
...
/**
 * 多形状图形, 目前支持的形状有：圆形、矩形、圆角矩形、正多边形，svg图像，正方形
 */
public class ShapeImageView extends BaseImageView {
    ...
    private float offsetAngle; // 多边形的起始偏移角度
     ...
     */
    public ShapeImageView(Context context, int resourceId, int shape, float rx, float ry,
            int sides, float offsetAngle, int svgRawResourceId) {
        this(context);
...
class SVG {
     ...
     * parser.
     * 
     * @param limits the bounds computed while parsing the SVG, may not be
     *            entirely accurate.
     */
...
class SVGParser {
    ...
    public static SVG getSVGFromString(String svgData, int searchColor, int replaceColor)
            throws SVGParseException {
        return SVGParser.parse(new ByteArrayInputStream(svgData.getBytes()), searchColor,
                replaceColor, false);
    }
    ...
    private static SVG parse(InputStream in, Integer searchColor, Integer replaceColor,
            boolean whiteMode, int targetWidth, int targetHeight) throws SVGParseException {
        ...
        public void endElement(String namespaceURI, String localName, String qName)
            ...
            if (localName.equals("svg")) {
                ...
                if (gradient.id != null) {
                    ...
                    LinearGradient g = new LinearGradient(gradient.x1, gradient.y1, gradient.x2,
                ...
                if (gradient.id != null) {
                    ...
                    RadialGradient g = new RadialGradient(gradient.x, gradient.y, gradient.radius,
